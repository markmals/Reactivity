...
if [[ "$file_path" == */Specs/*.md ]] || [[ "$file_path" == */Features/*/*.md ]]; then
    ...
    refs=$(rg -l --no-heading "SPEC:[[:space:]]*${spec_id}\b" Sources/ Tests/ 2>/dev/null || true)
    if [[ -n "$refs" ]]; then
        notes="Spec '$spec_id' was edited. Implementation referencing it may now drift:
$refs

Consider running /sdd-apply $spec_id to re-align the code, or /sdd-drift to see what drifted."
    fi
fi
