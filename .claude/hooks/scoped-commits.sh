...
if [ -f "$root/.claude/commit-scopes" ]; then
  # Strip inline/full-line `#` comments and surrounding whitespace; keep one bare
  # scope per non-empty line.
  allowed+=$'\n'$(sed -E 's/#.*$//; s/^[[:space:]]+//; s/[[:space:]]+$//' "$root/.claude/commit-scopes" 2>/dev/null | grep -vE '^$' || true)
fi
allowed+=$'\n'$(
