---
description: Adversarially review a spec's Swift implementation — try to break it.
...
2. **Locate the implementation and tests.** `rg "SPEC: <spec-id>"` in `Sources/`. Record the implementing files; find the tests tagged with the spec ID (`.spec("<spec-id>")` on the `@Suite`) and its scenario sub-IDs (`.scenario("<scenario-id>")` on each `@Test`).
...
3. **Confirm the suite is green first.** Run `mise run test` (filtered to the spec ID where the runner allows). If tests are **red**, stop and report it — a failing suite is a different problem; route it to `systematic-debugging`, not to the adversary. The refutational pass assumes code that already passes its own checks, exactly as the third stage runs only after the two confirmatory reviews are ✅.
...
Manual and agent-driven, like the other `sdd-*` commands. `rg` to locate the reverse pointers, `mise run test` to confirm green, and a dispatched subagent to run the `adversarial-review` skill. No aggregation tooling yet.
