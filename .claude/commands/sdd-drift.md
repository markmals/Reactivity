...
You are detecting drift between the specs and the library's implementation.
...
4. **Unimplemented spec.** A spec exists with no reverse pointer.
5. **Untagged impl.** A file in `Sources/` has no `// SPEC:` annotation and is not marked `// SPEC: manual`.
