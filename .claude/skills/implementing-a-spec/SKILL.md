...
description: Use when implementing a spec in the Swift library — `/sdd-apply <id>`. Dispatches a fresh subagent per spec, runs spec-compliance review, code-quality review, then an adversarial refutational pass, and tracks progress as it goes. This is the default "how to write code" workflow for this repo.
---

# Implementing a Spec

Each spec ID is a unit of work. To implement one, you dispatch a fresh subagent that writes failing tests, makes them pass, attaches a `// SPEC: <id>` reverse pointer in `Sources/`, then submits for review: spec compliance, code quality, then an adversarial pass that assumes the code is broken and tries to break it (the `adversarial-review` skill).
...
- Read the spec(s) and the existing code near where the new code will live
...
```
For each spec ID:
  1. Read spec + depends-on chain
  2. Identify existing reverse pointers, tests, and gaps in Sources/ and Tests/
...
3. Confirm there are tests covering **every Gherkin scenario** in the spec, each carrying the right `.scenario("<id>")` trait.
...
Same model, same general approach. Provide the same files plus the `swift-development` skill's idioms section.

Instructions:

1. Check **idioms**: is it idiomatic Swift (value semantics, correct Swift 6 isolation, minimal public surface) per `swift-development`?
...
Natural boundaries per spec applied (Scoped Commits — the **scope is the spec ID**, per `.claude/rules/commit-discipline.md`):

- **Test commit:** `<spec-id>: add failing scenarios` — the tests that pin the spec.
- **Implementation commit:** `<spec-id>: implement <behavior>` — the minimum code that makes the tests pass, with the `// SPEC: <id>` reverse pointer attached.
...
Fixups from review (either spec-compliance or code-quality) belong in the same commit pair if not yet pushed; otherwise land them as a follow-up commit under the same spec-ID scope.
...
- `/sdd-reconcile` — uses the spec/impl relationship in reverse: bring the spec into line with the current implementation
