...
- **Regression test + fix:** one commit. Subject: `<spec-id>: fix <bug description>` (scope is the spec ID the bug touches, or `swift` if none — Scoped Commits, see `.claude/rules/commit-discipline.md`). Body explains the root cause uncovered in Phase 1, not the symptom.
- **Split into two commits** when the failing test is independently valuable (e.g. it pins behavior that wasn't previously tested): `<spec-id>: add regression test` then `<spec-id>: fix <bug>`.
