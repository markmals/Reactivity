...
2. **Identify intent**: read the most recently touched specs, the latest commit messages, and the root `CLAUDE.md`. Synthesize the WHY of this pass.
3. **Run verification commands**:
   - `mise run build` — compile the library
   - `mise run test` — the Swift Testing suite (`swift test`)
   - `mise run lint` — swift-format strict lint Capture ✅/❌ per command and the failing-test count if any.
4. **Search the diff** for:
   - `[NEEDS CLARIFICATION]` markers introduced or unresolved
   - `TODO`, `FIXME`, `// SPEC: ... (deviates: ...)` annotations in `Sources/`
   - Entries added to the root `DEFECTS.md`
...
- **Don't include credentials, environment variables, or PII.** Reference them by name but never paste values.
- **Don't claim ✅ on a verification you didn't run.** If a command is too slow to run in your time budget, mark it `⏭ skipped` with a note, not ✅.
