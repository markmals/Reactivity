...
- `id` present, matches the file's slug, follows kind-prefix convention (`domain.*`, `story.*`, `usecase.*`, `error.*`, etc. — see CONVENTIONS.md for the taxonomy)
- `kind` present and one of the allowed values. The taxonomy is `narrative`, `story`, `use-case`, `domain`, `error`, `architecture`, `conventions`. **There is no `view-model`, `flow`, or `design-system` kind** — this is a non-UI library; flag any such kind as invalid.
...
- For `story.*` specs: every scenario has `Given/When/Then` and a stable sub-ID in an HTML comment (`<!-- id: scenario.<feature>.<capability>.<short-name> -->`)
...
- `rg "SPEC:[[:space:]]*<this-id>\b" Sources/ReactiveGraph` to find implementations. Report:
  ...
  - If the spec is behavior-bearing (`domain.*`, `story.*`, `error.*`) and has no pointer, flag the implementation gap
